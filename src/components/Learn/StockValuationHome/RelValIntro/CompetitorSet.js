    ...
    if (inputValue.length > 1) {
      try {
        ...
        if (response.ok) {
          ...
          const formattedData = data.map(item => ({ value: `${item.name} (${item.symbol})`, symbol: item.symbol }));
    ...
    <Box>
      <Autocomplete
        ...
        onChange={(value) => {
          ...
          const selected = suggestions1.find(suggestion => suggestion.value === value);
      ...
      <Autocomplete
        ...
        onChange={(value) => {
          ...
          const selected = suggestions2.find(suggestion => suggestion.value === value);
