...
const EVtoEBITDA = () => {
        ...
        if (cachedData) {
          ...
          try {
            const response1 = await fetch(
              `https://financialmodelingprep.com/api/v3/key-metrics-ttm/${stock1}?limit=40&apikey=${process.env.NEXT_PUBLIC_FIN_MOD_API_KEY}`,
            );
            const data1 = await response1.json();
            setEnterpriseValueMultiple1(data1[0].enterpriseValueOverEBITDATTM);

            const response2 = await fetch(
              `https://financialmodelingprep.com/api/v3/key-metrics-ttm/${stock2}?limit=40&apikey=${process.env.NEXT_PUBLIC_FIN_MOD_API_KEY}`,
            );
            const data2 = await response2.json();
            setEnterpriseValueMultiple2(data2[0].enterpriseValueOverEBITDATTM);

            const response3 = await fetch(
              `https://financialmodelingprep.com/api/v3/key-metrics-ttm/${stockSymbol}?limit=40&apikey=${process.env.NEXT_PUBLIC_FIN_MOD_API_KEY}`,
            );
  ...
  useEffect(() => {
    if (
      stockSymbol &&
      enterpriseValueMultiple1 &&
      enterpriseValueMultiple2 &&
      enterpriseValueMultiple3
    ) {
      callOpenAIAPI2(
        stockSymbol,
        enterpriseValueMultiple3,
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        Enterprise Value over EBITDA Relative Analysis
      </div>
      <div className={styles.content}>
        <div className={styles.row}>
          <Image
            src={logos[stockSymbol]}
          ...
          />
          <span>
            {stockSymbol} EV/EBITDA:{" "}
            {parseFloat(enterpriseValueMultiple3).toFixed(2)}
          </span>
        </div>
        <div className={styles.row}>
          <Image
          ...
          <span>
            {stock1} EV/EBITDA:{" "}
            {parseFloat(enterpriseValueMultiple1).toFixed(2)}
          </span>
        </div>
        <div className={styles.row}>
          <Image
          ...
          <span>
            {stock2} EV/EBITDA:{" "}
            {parseFloat(enterpriseValueMultiple2).toFixed(2)}
          </span>
        </div>
        {loading ? (
