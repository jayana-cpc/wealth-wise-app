...
export function Question2() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const storedAttempts =
        parseInt(localStorage.getItem("question2Attempts")) || 0;
  ...
  const handleSubmit = async () => {
    ...
    const APIBody = {
      ...
      messages: [
        {
          role: "system",
          content: `Evaluate the following answer for the question about being overwhelmed by student loan debt and considering different repayment options using the PACED decision-making process. If the answer is correct, respond with "Correct". If the answer is incorrect, respond with "Incorrect" and provide a helpful response but do not reveal answer..`,
        },
        {
          role: "user",
          content: userInput,
  ...
  return (
    <div>
      ...
      <Text>
        You&rsquo;re overwhelmed by your student loan debt and considering
        different repayment options. Some options offer lower monthly payments
        but extend the repayment period, while others have higher monthly
        payments but are paid off quicker. What PACED step(s) should you take
        FIRST to approach this decision effectively?
      </Text>
      <Space h="sm" />
      ...
      >
        <Text>Define the debt and your goals for repayment (Problem).</Text>
      </Modal>
    </div>
  );
}
