...
export function HomeownersInsurance() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      setAttempts(parseInt(localStorage.getItem('homeownersInsuranceAttempts')) || 0);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (aiFeedback.startsWith('Correct')) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('homeownersInsuranceFeedback', 'Correct! Great job!');
        }
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('homeownersInsuranceFeedback', 'Unexpected response from AI. Please try again.');
        }
      }
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('homeownersInsuranceFeedback', 'There was an error processing your answer. Please try again.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      ...
      <Space h="sm" />
      <Text><strong>Scenario: </strong>Sarah has recently purchased a new home and is considering her homeowners insurance options. Her insurer offers her a choice between a $500 deductible with a premium of $1,200 annually or a $1,000 deductible with a premium of $950 annually. Sarah is financially comfortable with higher out-of-pocket expenses in the event of a claim.
      </Text>
      <Space h="sm" />
      <Text>
        Given Sarah&apos;s comfort with higher deductibles and the potential for lower premiums, what should she consider when deciding between the two deductible options? How might her choice impact her financial situation in case of a claim?
      </Text>
      <Space h="sm" />
      ...
      {attempts >= 3 && (
        <>
          <Modal opened={modalOpened} onClose={() => setModalOpened(false)} title="Example Answer">
            <Text>
              Sarah should evaluate the potential savings of $250 on the premium by choosing the $1,000 deductible option against the higher out-of-pocket expense if a claim is made. If she is comfortable with the possibility of paying more in the event of a claim and prefers to save on premiums now, the higher deductible could be a good choice. She should ensure that she has sufficient savings to cover the $1,000 deductible in case of a claim.
            </Text>
          </Modal>
