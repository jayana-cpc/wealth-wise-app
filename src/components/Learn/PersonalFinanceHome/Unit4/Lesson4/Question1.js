...
export function Problem1() {
  ...
  const [correctAnswer, setCorrectAnswer] = useState('Tom will have approximately $6,083.25 in his savings account.');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (aiFeedback.startsWith('Correct')) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('problem1Feedback', 'Unexpected response from AI. Please try again.');
        }
      }
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('problem1Feedback', 'There was an error processing your answer. Please try again.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      ...
      <Text>
        Tom invests $5,000 in a savings account that earns an annual interest rate of 4%, compounded annually. 
      ...
      {attempts >= 3 && (
        <>
          <Modal opened={modalOpened} onClose={() => setModalOpened(false)} title="Example Answer">
            <Text>After 5 years, Tom will have approximately $6,083.25 in his savings account.</Text>
          </Modal>
