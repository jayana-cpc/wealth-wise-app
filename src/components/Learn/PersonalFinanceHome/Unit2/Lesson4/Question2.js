...
export function Question2() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (aiFeedback.startsWith('Correct')) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('question13Feedback', 'Unexpected response from AI. Please try again.');
        }
      }
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('question13Feedback', 'There was an error processing your answer. Please try again.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      ...
      <Text>
        Your employer has provided you with a W-2 that contains errors in your Social Security number and total wages earned.
        Outline the steps you would take to correct these errors and obtain a corrected W-2.
      </Text>
      <Space h="sm" />
      ...
      >
        ...
        <List>
          <List.Item>Contact your employer&apos;s payroll department immediately to report the errors.</List.Item>
          <List.Item>Provide necessary documentation, such as your Social Security card or pay stubs, to verify the correct information.</List.Item>
          <List.Item>Request a corrected W-2 form from your employer.</List.Item>
          <List.Item>Keep a copy of both the original and corrected W-2s for your records.</List.Item>
          <List.Item>If you have already filed your tax return, you may need to file an amended return (Form 1040X) to reflect the correct information.</List.Item>
        </List>
      </Modal>
    </div>
  );
