...
export function Question1() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (aiFeedback.startsWith('Correct')) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('question14Feedback', 'Unexpected response from AI. Please try again.');
        }
      }
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('question14Feedback', 'There was an error processing your answer. Please try again.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      ...
      <Space h="sm" />
      <Text>Sarah and Michael are a married couple filing jointly for tax year 2023. They incurred the following expenses:</Text>
      <List withPadding>
      ...
      <Text>The standard deduction for Married Filing Jointly is $25,900. Should Sarah and Michael itemize their deductions or take the standard deduction? Justify your answer based on their total itemized deductions and the standard deduction amount.
      </Text>
      <Space h="sm" />
      ...
      >
        <Text>Sarah and Michael should itemize their deductions. Here&apos;s the reasoning:</Text>
        <List>
          <List.Item>Total itemized deductions = $15,000 (mortgage interest) + $8,000 (state income taxes) + $3,500 (charitable contributions) + $5,000 (medical expenses) = $31,500</List.Item>
          <List.Item>Standard deduction for Married Filing Jointly = $25,900</List.Item>
        </List>
        <Text>
          Since their total itemized deductions ($31,500) exceed the standard deduction amount ($25,900), Sarah and Michael would benefit from itemizing their deductions. Itemizing allows them to reduce their taxable income by a larger amount, potentially lowering their tax liability or increasing their tax refund compared to taking the standard deduction.
        </Text>
      </Modal>
    </div>
  );
