...
import { Title, Text, Input, Button, Space, Modal, List, ListItem } from '@mantine/core';
...
export function Question2() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (aiFeedback.startsWith('Correct')) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('question15Feedback', 'Unexpected response from AI. Please try again.');
        }
      }
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('question15Feedback', 'There was an error processing your answer. Please try again.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      ...
      <Text>Emily is a single taxpayer with the following expenses for tax year 2023:</Text>
      <List withPadding>
      ...
      <Text>The standard deduction for Single filers is $12,950. Calculate Emily&apos;s total itemized deductions and determine whether she should itemize deductions or take the standard deduction. Explain your reasoning based on Emily&apos;s eligible expenses and the standard deduction amount for Single filers.</Text>
      <Space h="sm" />
      ...
      >
        <Text>Emily should take the standard deduction. Here&apos;s the reasoning:</Text>
        <List>
          <List.Item>Total itemized deductions = $7,500 (mortgage interest) + $4,000 (state income taxes) + $1,200 (charitable contributions) + $3,000 (medical expenses) = $15,700</List.Item>
          <List.Item>Standard deduction for Single filers = $12,950</List.Item>
        </List>
        <Text>
          Although Emily&apos;s total itemized deductions ($15,700) exceed the standard deduction amount ($12,950), she may benefit more from taking the standard deduction. This is because the difference between her itemized deductions and the standard deduction amount isn&apos;t substantial enough to significantly lower her taxable income. Taking the standard deduction simplifies her tax filing process and ensures she receives a predictable tax benefit without the need for additional documentation or potential limitations associated with itemized deductions.
        </Text>
      </Modal>
    </div>
  );
